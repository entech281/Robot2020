...
public class EncoderPoseGenerator implements PoseGenerator{
    ...
    @Override
    public void updateFromOfficialPose(PositionReader officialPose) {
        pose.setHorizontal(officialPose.getHorizontal());
        pose.setLateral(officialPose.getLateral());
        pose.setTheta(officialPose.getTheta());
    }
}